...
REQUIRED_IMAGE_FOR_PROPS = ['product', 'vendor', 'system', 'system_ext']
    ...
    class _PayloadHeader(object):
        ...
        def ReadFromPayload(self, payload_file):
            ...
            if self.version != BRILLO_MAJOR_PAYLOAD_VERSION:
                raise PayloadError(
                    'Unsupported payload version (%d)' % self.version)
            self.size += 4
            self.metadata_signature_len = struct.unpack(
                '>I', payload_file.read(4))[0]
